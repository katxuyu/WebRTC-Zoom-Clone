...
navigator.mediaDevices
    ...
    .then((stream) => {
        addVideoStream(myVideo, stream, 'Me', "1");
        ...
        myPeer.on("call", (call) => {
            ...
            call.on("stream", (userVideoStream) => {
                addVideoStream(video, userVideoStream, call.peer, "2");
            });
        });

        socket.on("user-connected", (userID, username) => {
            connectNewUser(userID, stream, username);
...
var count = 0;
const addVideoStream = (video, stream, username, o) => {
    ...
    count++;
    if (username == "Me" || count % 2 == 0){   
        track = video.addTextTrack("captions", "English", "en");
        track.mode = "showing";
        track.addCue(new VTTCue(0, 1000000, username));    
        videoGrid.append(video);
        count == 0;
    }
};

const connectNewUser = (userID, stream, username) => {
    const call = myPeer.call(userID, stream);
    const video = document.createElement("video");

    call.on("stream", (userVideoStream) => {
        addVideoStream(video, userVideoStream, username, '3');
    });
...
const handleLeave = () => {
    ...
    location.href='https://gie-zoom.herokuapp.com/';
